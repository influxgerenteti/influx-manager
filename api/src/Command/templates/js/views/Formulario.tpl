...
export default {
  ...
  computed: {
    ...mapState('$(nomeModuloSensitiveCase)', ['itemSelecionado', 'itemSelecionadoID', 'estaCarregando'])
  },
  mounted () {
  ...
  methods: {
    ...mapMutations('$(nomeModuloSensitiveCase)', ['SET_ITEM_SELECIONADO_ID', 'LIMPAR_ITEM_SELECIONADO', 'SET_ESTA_CARREGANDO']),
    ...mapActions('$(nomeModuloSensitiveCase)', ['buscar', 'criar', 'atualizar']),

    voltar () {
      this.LIMPAR_ITEM_SELECIONADO()
      this.$router.push('/$(nomeModuloPai)/$(nomePasta)')
    },
