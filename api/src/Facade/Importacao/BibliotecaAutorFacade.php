...
use App\BO\Importacao\BibliotecaAutorBO;
 ...
 * @author Luiz A Costa
 */
class BibliotecaAutorFacade extends GenericFacade
{
    /**
     *
    ...
    public function criar(&$mensagemErro, $franqueada_id, &$xlsxHelper)
    {
        if (FranqueadaBO::franqueadaExisteBanco($this->foundationEntityManager->getRepository(Franqueada::class), ["id" => $franqueada_id]) === true) {
            BibliotecaAutorBO::excluirRegistrosPorFranqueada($this->bibliotecaAutorRepository, self::getEntityManager(), $franqueada_id);
            BibliotecaAutorBO::criarRegistrosPorFranqueada($xlsxHelper, self::getEntityManager(), $franqueada_id);
        } else {
