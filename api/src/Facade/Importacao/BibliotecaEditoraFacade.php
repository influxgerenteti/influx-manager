...
use App\BO\Importacao\BibliotecaEditoraBO;
 ...
 * @author Luiz A Costa
 */
class BibliotecaEditoraFacade extends GenericFacade
{
    ...
    public function criar(&$mensagemErro, $franqueada_id, &$xlsxHelper)
    {
        if (FranqueadaBO::franqueadaExisteBanco($this->foundationEntityManager->getRepository(Franqueada::class), ["id" => $franqueada_id]) === true) {
            BibliotecaEditoraBO::excluirRegistrosPorFranqueada($this->bibliotecaEditoraRepository, self::getEntityManager(), $franqueada_id);
            BibliotecaEditoraBO::criarRegistrosPorFranqueada($xlsxHelper, self::getEntityManager(), $franqueada_id);
        } else {
