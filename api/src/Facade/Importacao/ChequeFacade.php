...
use App\BO\Importacao\ChequeBO;
...
class ChequeFacade extends GenericFacade
{
    ...
    /**
     *
     * @var \App\Repository\Importacao\ChequeRepository
     */
    ...
    public function criar(&$mensagemErro, $franqueada_id, &$xlsxHelper)
    {
        if (FranqueadaBO::franqueadaExisteBanco($this->foundationEntityManager->getRepository(Franqueada::class), ["id" => $franqueada_id]) === true) {
            ChequeBO::excluirRegistrosPorFranqueada($this->chequeRepository, self::getEntityManager(), $franqueada_id);
            ChequeBO::criarRegistrosPorFranqueada($xlsxHelper, self::getEntityManager(), $franqueada_id, $this->alunoRepository, $this->funcionarioRepository, $this->empresaRepository);
        } else {
