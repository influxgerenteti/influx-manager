...
use App\Helper\RemessaPHP\Cnab\Factory;
...
class YamlLoad
{
    ...
    {
        if (empty($array) === true || empty($array['generic']) === true) {
    ...
    public function loadArray(Linha $cnabLinha, $array)
    {
        ...
        $keys = ['generic'];
        if (array_key_exists(sprintf('%03d', $this->codigo_banco), $array) === true) {
    ...
    {
        if (isset(static::$loaded[$filename]) === false) {
    ...
    public function loadFormat($cnab, $filename)
    {
        ...
        if (file_exists($filenamePadrao) === false && file_exists($filenameEspecifico) === false) {
        ...
        if (empty($arrayPadrao) === false) {
            $arrayFormat['generic'] = $arrayPadrao;
        }

        if (empty($arrayEspecifico) === false) {
