...
class Arquivo implements \App\Helper\RemessaPHP\Cnab\Remessa\IArquivo
{
    ...
    {
        $banco  = \App\Helper\RemessaPHP\Cnab\Banco::getBanco($this->codigo_banco);
        $campos = [
            'data_geracao',
            'data_gravacao',
            'nome_fantasia',
        ...
        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::CEF) {
            $campos[] = 'codigo_cedente';
        }

        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::BANCO_DO_BRASIL) {
        ...
        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::CEF || $this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::BANCO_DO_BRASIL) {
            $campos[] = 'agencia';
            $campos[] = 'agencia_dv';
            $campos[] = 'conta';
            $campos[] = 'operacao';
        ...
        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::SICREDI) {
            $campos[] = 'agencia';
            $campos[] = 'agencia_dv';
            $campos[] = 'numero_sequencial_arquivo';
            $campos[] = 'codigo_convenio';
        ...
        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::SICREDI) {
            ...
            // $this->headerLote->codigo_carteira = $this->configuracao['codigo_carteira'];
        }
    ...
    public function insertDetalhe(array $boleto, $tipo='remessa')
    {
        ...
        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::SICREDI) {
            $detalhe->segmento_p->codigo_cedente          = $this->headerArquivo->codigo_cedente;
            $detalhe->segmento_p->agencia_mais_cedente_dv = $this->headerArquivo->agencia_mais_cedente_dv;
            $detalhe->segmento_p->codigo_carteira         = $this->configuracao['codigo_carteira'];
        }

        $detalhe->segmento_p->nosso_numero = $this->formatarNossoNumero($boleto['nosso_numero'], $boleto['agencia'], $boleto['conta']);
        ...
        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::BANCO_DO_BRASIL) {
            ...
            if ($boleto['carteira'] === 17 && $boleto['codigo_carteira'] === \App\Helper\RemessaPHP\Cnab\CodigoCarteira::COBRANCA_SIMPLES) {
        ...
        if ($this->layoutVersao === 'sigcb' && $this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::CEF) {
        ...
        if (empty($boleto['registrado']) === false || $this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::SICREDI) {
            $detalhe->segmento_p->forma_cadastramento = 1;
        } else {
            $detalhe->segmento_p->forma_cadastramento = 2;
        }

        // 1 = Com, 2 = Sem Registro
        if (empty($boleto['registrado']) === false && $this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::CEF) {
        ...
        $detalhe->segmento_p->especie          = $boleto['especie'];
        // 4 = Duplicata serviço
        ...
        $detalhe->segmento_p->codigo_juros_mora = 1;
        // 1 = Por dia
        if (empty($boleto['dias_iniciar_contagem_juros']) === false && is_numeric($boleto['dias_iniciar_contagem_juros']) === true) {
        ...
        if (empty($boleto['codigo_protesto']) === false && empty($boleto['prazo_protesto']) === false) {
        ...
        } else if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::SICREDI) {
            $detalhe->segmento_p->codigo_baixa = 1;
        } else {
            if (isset($boleto['baixar_apos_dias']) === true) {
        ...
        if (array_key_exists('identificacao_distribuicao', $boleto) === true) {
        ...
        if (empty($boleto['sacado_cnpj']) === false) {
        ...
        if ($detalhe->segmento_r->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::SICREDI) {
            // Conforme PDF enviado com correções em 26/05/21, a Sicredi só aceita o valor da taxa de multa em percentuais (código 2)
    ...
    public function formatarNossoNumero($nossoNumero, $agencia=null, $conta=null)
    {
        if (empty($nossoNumero) === true)
            return $nossoNumero;
        ...
        if ($this->codigo_banco === \App\Helper\RemessaPHP\Cnab\Banco::BANCO_DO_BRASIL) {
            ...
            } else if (strlen($codigo_convenio) <= 6) {
            ...
            $nossoNumero = BoletoFacade::geraNossoNumeroSicredi($agencia, $conta, $nossoNumero);
        }//end if
    ...
    public function getText()
    {
        ...
        if ($this->trailerLote->validate() === false) {
            throw new \InvalidArgumentException($this->trailerLote->last_error);
        }

        if ($this->trailerArquivo->validate() === false) {
