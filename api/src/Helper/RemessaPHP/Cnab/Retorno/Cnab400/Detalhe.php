...
{
    private $tipo_baixa_bb        = [6];
    private $tipo_baixa_bradesco  = [
        6,
        11,
        15,
        16,
    ...
    private $tipo_baixa_rejeitada = [15];
    public $codigo_banco;
    ...
    {
        $tipo_baixa = $this->tipo_baixa_padrao;
        switch ($this->codigo_banco) {
            case \App\Helper\RemessaPHP\Cnab\Banco::CEF:
            $tipo_baixa = $this->tipo_baixa_cef;
                break;
            case \App\Helper\RemessaPHP\Cnab\Banco::BANCO_DO_BRASIL:
            $tipo_baixa = $this->tipo_baixa_bb;
                break;
            case \App\Helper\RemessaPHP\Cnab\Banco::BRADESCO:
            $tipo_baixa = $this->tipo_baixa_bradesco;
                break;
        }

        return in_array($this->getCodigo(), $tipo_baixa);
    }
    ...
    /**
     * Retorna o valor dos juros (antes da emissão).
     *
     * @return Double;
     */
    ...
    {
        if (\App\Helper\RemessaPHP\Cnab\Banco::CEF === $this->codigo_banco) {
            return 0;
        }
    ...
    {
        $data           = false;
        $dataVencimento = sprintf('%06d', $this->data_vencimento);
        if ($dataVencimento !== "000000") {
            $data = \DateTime::createFromFormat('dmy', $dataVencimento);
            $data->setTime(0, 0, 0);
        }
    ...
    {
        $data        = false;
        $dataCredito = sprintf('%06d', $this->data_credito);
        if ($dataCredito !== "000000") {
            $data = \DateTime::createFromFormat('dmy', $dataCredito);
            $data->setTime(0, 0, 0);
        }
    ...
    {
        $data           = false;
        $dataOcorrencia = sprintf('%06d', $this->data_de_ocorrencia);
        if ($dataOcorrencia !== "000000") {
            $data = \DateTime::createFromFormat('dmy', $dataOcorrencia);
            $data->setTime(0, 0, 0);
        }
    ...
    {
        if ($this->existField('codigo_liquidacao') === true) {
    ...
    public function getDescricaoLiquidacao()
    {
        ...
        if (\App\Helper\RemessaPHP\Cnab\Banco::ITAU === $this->codigo_banco) {
            $tabela = [
        ...
        if (array_key_exists($codigoLiquidacao, $tabela) === true) {
    ...
    {
        if ($this->existField('boleto_dda') === true) {
            if (empty($this->boleto_dda) === true) {
                return false;
            }
        ...
        if ($this->codigo_banco === 341) {
            if ($this->getCodigo() === 25) {
                ...
                foreach ($alegacoes as $alegacao) {
                    if (array_key_exists($alegacao, $tabelaAlegacao) === true) {
