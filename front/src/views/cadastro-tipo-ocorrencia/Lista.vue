<template>
  <div class="animated fadeIn">
    <div class="d-flex justify-content-end mb-2">
      <router-link v-if="permissoes['CRIAR'] && (permissoes['CRIAR'].possui_permissao === true)" to="/cadastros/tipo-ocorrencia/adicionar" class="btn btn-azul">
      ...
      <g-table :sort="sortTable">
        <thead class="text-dark">
          <tr>
            ...
            <th data-column="oc.tipo_operacao">Tipo de Ocorrência</th>
        ...
        <tbody ref="scroll-wrap">
          <perfect-scrollbar @ps-y-reach-end="permitirCarregarMais && carregarMais()">
            ...
            <tr v-for="item in listaItems" :key="item.id" @dblclick="alterar(item)">
              ...
              <td data-label="Tipo de Ocorrência">{{ item.tipo }}</td>
              <td data-label="Situação">
                <div @click.prevent="inativar(item)">
                  <span v-b-tooltip.viewport.left.hover v-if="item.situacao == 1" class="align-middle text-success" title="Desativar"><font-awesome-icon icon="check-square" /></span>
              ...
              <td class="d-flex coluna-icones">
                <a v-if="permissoes['EDITAR'] && (permissoes['EDITAR'].possui_permissao === true)" :class="item.situacao == 1 ? null : 'disable-icon'" href="javascript:void(0)" class="icone-link" title="Atualizar" @click.prevent="alterar(item)">
...
export default {
  ...
  computed: {
    ...
    ...mapState('cadastroTipoOcorrencia', {listaItems: 'lista', estaCarregando: 'estaCarregando', todosItensCarregados: 'todosItensCarregados', totalItens: 'totalItens'}),
  ...
  methods: {
    ...mapActions('cadastroTipoOcorrencia', {listarItens: 'listar', atualizar: 'atualizar'}),
    ...mapMutations('cadastroTipoOcorrencia', ['SET_PAGINA_ATUAL', 'SET_ITEM_SELECIONADO', 'SET_ITEM_SELECIONADO_ID', 'LIMPAR_ITEM_SELECIONADO', 'SET_LISTA', 'SET_ORDER_BY']),
    ...
    alterar (item) {
      if (this.permissoes['EDITAR'] && (this.permissoes['EDITAR'].possui_permissao === true)) {
        ...
        this.$router.push(`/cadastros/tipo-ocorrencia/atualizar/${item.id}`)
      }
    },

    inativar (item) {
      const mensagem = item.situacao == 1 ? 'desativar' : 'ativar'
      EventBus.$emit('chamarModal', {
        resolve: success => {
          this.SET_ITEM_SELECIONADO_ID(item.id)

          const data = Object.assign({}, item)
          data.situacao = item.situacao == 1 ? 0 : 1
