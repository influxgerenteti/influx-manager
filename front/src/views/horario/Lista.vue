<template>
  <div class="animated fadeIn">
    <div class="d-flex justify-content-end mb-4">
      <router-link v-if="permissoes['CRIAR'] && (permissoes['CRIAR'].possui_permissao === true)" to="/configuracoes/horario/adicionar" class="btn btn-azul">
    ...
    <div class="table-responsive-sm">
      <g-table :sort="sortTable">
        <thead>
          <tr>
            <th data-column="hr.descricao">Descrição</th>
            <th data-column="hr.situacao">Situação</th>
        ...
        <tbody>
          <perfect-scrollbar>
            ...
            <tr v-for="item in lista" :key="item.id">
              ...
              <!-- <td v-if="item.situacao === 'A'" class="text-success"><span class="badge border-badge align-middle rounded">Ativo</span></td>
              <td v-else class="icon-danger"><span class="badge border-badge align-middle rounded">Inativo</span></td> -->
...
export default {
  computed: {
    ...
    ...mapState('horario', ['lista', 'estaCarregando', 'todosItensCarregados', 'totalItens']),
  ...
  methods: {
    ...mapActions('horario', ['listar', 'atualizar']),
    ...mapMutations('horario', ['SET_PAGINA_ATUAL', 'SET_ESTA_CARREGANDO', 'SET_ITEM_SELECIONADO', 'SET_ITEM_SELECIONADO_ID', 'SET_ORDER_BY', 'SET_LISTA']),
    ...
    mostrar (item) {
      this.$router.push(`/configuracoes/horario/visualizar/${item.id}`)
    },
    ...
    alterar (item) {
      if (this.permissoes['EDITAR'] && (this.permissoes['EDITAR'].possui_permissao === true)) {
        ...
        this.$router.push(`/configuracoes/horario/atualizar/${item.id}`)
      }
    },
