...
export default {
  name: 'FormularioImportacaoBoleto',
  data () {
    return {
      isValid: true,
      isEdit: false
  ...
  computed: {
    ...mapState('importacaoBoleto', ['itemSelecionado', 'itemSelecionadoID', 'estaCarregando'])
  },
  mounted () {
  ...
  methods: {
    ...mapMutations('importacaoBoleto', ['SET_ITEM_SELECIONADO_ID', 'LIMPAR_ITEM_SELECIONADO', 'SET_ESTA_CARREGANDO']),
    ...mapActions('importacaoBoleto', ['buscar', 'criar', 'atualizar']),

    voltar () {
      this.LIMPAR_ITEM_SELECIONADO()
      this.$router.push('/financeiro/importacao-boleto')
    },
