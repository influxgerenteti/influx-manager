...
export default {
  name: 'FormularioRelatorioBalanceteFinanceiro',
  data () {
    return {
      isValid: true,
      isEdit: false
  ...
  computed: {
    ...mapState('relatorioBalanceteFinanceiro', ['itemSelecionado', 'itemSelecionadoID', 'estaCarregando'])
  },
  mounted () {
  ...
  methods: {
    ...mapMutations('relatorioBalanceteFinanceiro', ['SET_ITEM_SELECIONADO_ID', 'LIMPAR_ITEM_SELECIONADO', 'SET_ESTA_CARREGANDO']),
    ...mapActions('relatorioBalanceteFinanceiro', ['buscar', 'criar', 'atualizar']),

    voltar () {
      this.LIMPAR_ITEM_SELECIONADO()
      this.$router.push('/relatorios/relatorio-balancete-financeiro')
    },
