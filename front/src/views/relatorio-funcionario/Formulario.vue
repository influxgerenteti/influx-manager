...
export default {
  name: 'FormularioRelatorioFuncionario',
  data () {
    return {
      isValid: true,
      isEdit: false
  ...
  computed: {
    ...mapState('relatorioFuncionario', ['itemSelecionado', 'itemSelecionadoID', 'estaCarregando'])
  },
  mounted () {
  ...
  methods: {
    ...mapMutations('relatorioFuncionario', ['SET_ITEM_SELECIONADO_ID', 'LIMPAR_ITEM_SELECIONADO', 'SET_ESTA_CARREGANDO']),
    ...mapActions('relatorioFuncionario', ['buscar', 'criar', 'atualizar']),

    voltar () {
      this.LIMPAR_ITEM_SELECIONADO()
      this.$router.push('/relatorio/relatorio-funcionario')
    },
