    ...
    <form :class="{ 'was-validated': !estaValido }" class="needs-validation" novalidate @submit.prevent="submit()">
      ...
      <div class="body-sector p-3">
        <b-row class="align-items-center">
          ...
          <b-col md="4">
            <label v-help-hint="'form-lotacao_maxima'" for="lotacao_maxima">Máximo de alunos *</label>
...
export default {
  ...
  computed: {
    ...
    ...mapState('sala', {listaSalas: 'lista'}),

    opcoesSalas: {
      get () {
        const existentes = this.salasFranqueada ? this.salasFranqueada.map(item => item.sala_id * 1) : []
        return this.listaSalas.filter(sala => !existentes.includes(sala.id))
      }
    }
  },
